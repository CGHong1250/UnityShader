    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                float4 _BaseMap_ST;
                half _Smoothness, _BumpScale, _Metallic,_Refraction;
            CBUFFER_END
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                half ratio = (1 - pow(dot(input.normalWS,viewWS),2.0)) * _Refraction;
                //我们要偏移还需要一个方向，让方向乘以比例。把法线方向转到视角空间，沿法线方向偏移，我们只要xy不需要z
                float3 refractionOffset = _Refraction * TransformWorldToViewDir(input.normalWS) * ratio;
                half4 offsetUV = SAMPLE_TEXTURE2D(_CameraOpaqueTexture ,sampler_CameraOpaqueTexture, screenUV + refractionOffset.xy);
                ...
                half4 color = UniversalFragmentPBR(inputData, surfaceData) * offsetUV;
