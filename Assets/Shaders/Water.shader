Shader "Hong/Water"
{
    ...
    { 
        ...
        _FlowMapTiling ("FlowMapTiling", Float) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                //float2 _Direction;
                float _UJump, _VJump, _FlowMapTiling, _Speed, _FlowStrength;
                float _NormalTiling, _Refraction, _DepthDistance, _DepthFogDistance;
            ...
            float3 FlowUV (float2 uv, float2 jump, float tiling, float speed, float flowStrength, float time, bool flowB)
            {
                float3 uvw;
                float phaseOffset = flowB ? 0.5 : 0;
                float4 flowMap =  SAMPLE_TEXTURE2D(_FlowMap, sampler_FlowMap,uv );
            ...
            //水面折射
            float3 RefractionColor(float4 positionCS, float3 normalWS, float refraction)
            {
                //float3 normal = TransformWorldToViewDir(normalWS);
                float3 normal =normalWS;
                float2 uvOffset = normal.xz * refraction;
                float2 uv = positionCS.xy + uvOffset/ positionCS.w;
                //float2 uv = screenUV + uvOffset;
                float3 refractionColor = SAMPLE_TEXTURE2D(_CameraOpaqueTexture ,sampler_CameraOpaqueTexture, uv).rgb;
            ...
            float DepthFog (float3 positionWS, float Depth)
            {
                ...
                return saturate(depthDifference/Depth);
            }
            ...
            inline void MaterialInputData(float time, float speed, float flowStrength, float flowMapTiling, float normalTiling, float2 jump, float depthFog, float depthWater, half4 shallowWater, half4 deepWater, out MaterialInput outMaterialInput)
            {
                ...
                outMaterialInput.flowMapTiling = flowMapTiling;
            ...
            {
                float3 uvwA = FlowUV(uv, MaterialInput.jump, MaterialInput.flowMapTiling, MaterialInput.speed, MaterialInput.flowStrength, MaterialInput.time, false);
                float3 uvwB = FlowUV(uv, MaterialInput.jump, MaterialInput.flowMapTiling, MaterialInput.speed, MaterialInput.flowStrength, MaterialInput.time, true);
                float2 uvA = uvwA.xy;
                float2 uvB = uvwB.xy;
                
                half4 DepthColor = lerp(MaterialInput.deepWater, MaterialInput.shallowWater, MaterialInput.depthFog);
                ...
                outSurfaceData.albedo = albedoAlpha.rgb * _BaseColor.rgb * DepthColor.rgb;
                outSurfaceData.alpha = albedoAlpha.a * _BaseColor.a * max(0.25,MaterialInput.depthWater);
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                half depthFog = DepthFog(input.positionWS, _DepthFogDistance);
                half depthWater = DepthWater(input.positionCS, input.positionVS, _DepthDistance);

                MaterialInput MaterialInput;
                MaterialInputData(_Time.y, _Speed, _FlowStrength, _FlowMapTiling, _NormalTiling, jump, 
                                    depthFog, depthWater, _ShallowWater, _DeepWater, MaterialInput);

                SurfaceData surfaceData;
                InitializeStandardLitSurfaceData(input.uv, MaterialInput, surfaceData);
                
                //half3 refractionColor = RefractionColor(input.positionCS, input.normalWS, _Refraction);
